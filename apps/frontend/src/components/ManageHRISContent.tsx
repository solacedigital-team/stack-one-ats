...
import { listAccountsByCategory } from "../http/listAccounts"; // Assuming HRIS systems are also "accounts"
...
const ManageHRISContent: React.FC = () => {
	...
	const fetchHrisSystems = async () => {
		try {
			...
			const hrisData = await listAccountsByCategory("hris");
	...
	return (
		...
		>
			...
			{selectedHrisId && <ListEmployeesButton accountId={selectedHrisId} />}
			{showOnboardingConfig && <OnboardingConfigurationComponent />}
			{selectedHrisId && <OnboardUsersButton hrisId={selectedHrisId} />}
			<Contact />
		</div>
	);
};
